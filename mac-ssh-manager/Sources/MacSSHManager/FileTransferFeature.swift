...
private enum RemoteTransferCommand {
    ...
    private static func sshOptionArgs(host: HostEntry) -> [String] {
        ...
        args.append(contentsOf: ["-o", "StrictHostKeyChecking=\(host.putty.strictHostKeyChecking.sshValue)"])

        let timeout = max(0, host.putty.connectTimeoutSeconds)
        if timeout > 0 {
            args.append(contentsOf: ["-o", "ConnectTimeout=\(timeout)"])
        }

        let knownHosts = expandedLocalPath(host.putty.userKnownHostsFile.trimmed)
