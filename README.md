...
* Requirements
  ...
  - `brew install perl pkg-config gtk+3`
* Build CLI tools only (`plink`, `pscp`, `psftp`, `puttygen`)
  - `./scripts/build-macos-arm64.sh --cli-only`
* Prepare makefiles only (no compile)
  - `./scripts/build-macos-arm64.sh --prepare-only`
* Build GUI + CLI (GUI binary is GTK-based `putty`)
