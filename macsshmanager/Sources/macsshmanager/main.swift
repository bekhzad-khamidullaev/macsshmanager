...
struct HostEntry: Identifiable, Codable, Equatable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(sshSettings, forKey: .sshSettings)
...
final class HostStore: ObservableObject {
    ...
    init() {
        ...
        migrationHostsURL = appDir.appendingPathComponent("hosts.json")
        load()
    }
    ...
    func load() {
        ...
        if let migrationData = try? Data(contentsOf: migrationHostsURL),
...
final class TerminalRuntime: NSObject, ObservableObject, @preconcurrency LocalProcessTerminalViewDelegate {
    ...
    private func authList(for settings: SSHSettings) -> [String] {
    ...
    private func proxyCommand(for settings: SSHSettings) -> String? {
    ...
    func setTerminalTitle(source: LocalProcessTerminalView, title: String) {
        if host.sshSettings.disableRemoteTitle {
            return
        }
        let t = title.trimmed
        if !t.isEmpty {
...
enum SystemTerminalLauncher {
    ...
    private static func buildSSHCommand(host: HostEntry) -> Result<String, CommandBuildError> {
        ...
        var sshArgs: [String] = [
            ...
            "-o", "StrictHostKeyChecking=\(host.sshSettings.strictHostKeyChecking.sshValue)"
        ]
        ...
        let remoteCommand = host.sshSettings.remoteCommand.trimmed
        if !remoteCommand.isEmpty {
    ...
    private var terminalTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                row("Charset") {
                    TextField("UTF-8", text: $host.sshSettings.localeCharset)
                        .textFieldStyle(.roundedBorder)
                }
                row("Columns / Rows") {
                    HStack(spacing: 8) {
                        TextField("120", value: $host.sshSettings.terminalColumns, format: .number)
                            .textFieldStyle(.roundedBorder)
                            .frame(width: 90)
                        TextField("34", value: $host.sshSettings.terminalRows, format: .number)
                            .textFieldStyle(.roundedBorder)
                            .frame(width: 90)
                    }
                }
                row("Scrollback") {
                    TextField("10000", value: $host.sshSettings.scrollbackLines, format: .number)
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 140)
                }
                row("Font size") {
                    TextField("13", value: $host.sshSettings.fontSize, format: .number)
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 140)
                }
                row("Bell mode") {
                    Picker("", selection: $host.sshSettings.bellMode) {
                ...
                row("Console theme") {
                    Picker("", selection: $host.sshSettings.consoleTheme) {
                ...
                row("Alt as Meta") {
                    Toggle("", isOn: $host.sshSettings.optionIsMeta)
                        .labelsHidden()
                }
                row("Mouse reporting") {
                    Toggle("", isOn: $host.sshSettings.allowMouseReporting)
                        .labelsHidden()
                }
                row("Remote title") {
                    Toggle("Allow updates", isOn: Binding(
                        get: { !host.sshSettings.disableRemoteTitle },
                        set: { host.sshSettings.disableRemoteTitle = !$0 }
                    ))
                }
                row("Backspace key") {
                    Toggle("Send Ctrl+H", isOn: $host.sshSettings.backspaceSendsControlH)
                }
                row("Delete key") {
                    Toggle("Send DEL", isOn: $host.sshSettings.deleteSendsDEL)
                }
            }
            .padding(.top, 6)
        }
    ...
    private var connectionTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                row("Host key policy") {
                    Picker("", selection: $host.sshSettings.strictHostKeyChecking) {
                ...
                row("Known hosts file") {
                    TextField("~/.ssh/known_hosts", text: $host.sshSettings.userKnownHostsFile)
                        .textFieldStyle(.roundedBorder)
                }
                row("Connect timeout") {
                    TextField("15", value: $host.sshSettings.connectTimeoutSeconds, format: .number)
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 140)
                }
                row("Keepalive sec") {
                    TextField("0", value: $host.sshSettings.keepAliveIntervalSeconds, format: .number)
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 140)
                }
                row("Keepalive count") {
                    TextField("3", value: $host.sshSettings.keepAliveCountMax, format: .number)
                        .textFieldStyle(.roundedBorder)
                        .frame(width: 140)
                }
                row("TCP keepalive") {
                    Toggle("", isOn: $host.sshSettings.tcpKeepAlive)
                        .labelsHidden()
                }
                row("IP version") {
                    Picker("", selection: $host.sshSettings.addressFamily) {
                ...
                row("Bind address") {
                    TextField("Optional", text: $host.sshSettings.bindAddress)
                        .textFieldStyle(.roundedBorder)
                }
                row("File protocol") {
    ...
    private var authTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                row("Password auth") {
                    Toggle("", isOn: $host.sshSettings.enablePasswordAuth)
                        .labelsHidden()
                }
                row("Keyboard-interactive") {
                    Toggle("", isOn: $host.sshSettings.enableKeyboardInteractiveAuth)
                        .labelsHidden()
                }
                row("GSSAPI auth") {
                    Toggle("", isOn: $host.sshSettings.enableGSSAPIAuth)
                        .labelsHidden()
                }
                row("Forward agent") {
                    Toggle("", isOn: $host.sshSettings.forwardAgent)
                        .labelsHidden()
                }
                row("GSSAPI delegate creds") {
                    Toggle("", isOn: $host.sshSettings.gssapiDelegateCredentials)
                        .labelsHidden()
                }
                row("Compression") {
                    Toggle("", isOn: $host.sshSettings.compression)
                        .labelsHidden()
                }
                row("X11 forwarding") {
                    Toggle("", isOn: $host.sshSettings.x11Forwarding)
                        .labelsHidden()
                }
                if host.sshSettings.x11Forwarding {
                    row("X11 display") {
                        TextField("localhost:0", text: $host.sshSettings.x11Display)
                            .textFieldStyle(.roundedBorder)
                    }
                }
            }
    ...
    private var sshTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                row("Ciphers") {
                    TextField("chacha20-poly1305@openssh.com,...", text: $host.sshSettings.ciphers)
                        .textFieldStyle(.roundedBorder)
                }
                row("MACs") {
                    TextField("hmac-sha2-256-etm@openssh.com,...", text: $host.sshSettings.macs)
                        .textFieldStyle(.roundedBorder)
                }
                row("Host key algos") {
                    TextField("ssh-ed25519,ecdsa-sha2-nistp256,...", text: $host.sshSettings.hostKeyAlgorithms)
                        .textFieldStyle(.roundedBorder)
                }
                row("Rekey limit") {
                    TextField("1G 1h", text: $host.sshSettings.rekeyLimit)
                        .textFieldStyle(.roundedBorder)
                        .frame(maxWidth: 260)
                }
                row("Remote command") {
                    TextField("Optional", text: $host.sshSettings.remoteCommand)
                        .textFieldStyle(.roundedBorder)
                }
                row("No shell") {
                    Toggle("Use -N when no command", isOn: $host.sshSettings.noShell)
                }
            }
            .padding(.top, 6)
        }
    ...
    private var proxyTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if host.sshSettings.proxyType == .command {
                    row("Proxy command") {
                        TextField("nc -x proxy:1080 -X 5 %h %p", text: $host.sshSettings.proxyCommand)
                            .textFieldStyle(.roundedBorder)
                    }
                } else if host.sshSettings.proxyType != .none {
                    row("Proxy host") {
                        TextField("proxy.example.com", text: $host.sshSettings.proxyHost)
                            .textFieldStyle(.roundedBorder)
                    }
                    row("Proxy port") {
                        TextField("1080", value: $host.sshSettings.proxyPort, format: .number)
                            .textFieldStyle(.roundedBorder)
                            .frame(width: 140)
                    }
                    row("Proxy user") {
                        TextField("Optional", text: $host.sshSettings.proxyUsername)
                            .textFieldStyle(.roundedBorder)
                    }
                    row("Proxy password") {
                        SecureField("Optional", text: $host.sshSettings.proxyPassword)
                            .textFieldStyle(.roundedBorder)
                    }
                }
            }
    ...
    private var loggingTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                ...
                row("Verbosity") {
                    Picker("", selection: $host.sshSettings.logVerbosity) {
                ...
                row("Log file") {
                    TextField("~/Library/Logs/macsshmanager-session.log", text: $host.sshSettings.logFilePath)
                        .textFieldStyle(.roundedBorder)
                }
            }
            .padding(.top, 6)
...
struct ContentView: View {
    ...
    private func sanitizeHostDraft(_ host: inout HostEntry) {
        ...
        host.sshSettings.terminalType = host.sshSettings.terminalType.trimmed
        host.sshSettings.localeCharset = host.sshSettings.localeCharset.trimmed
        host.sshSettings.userKnownHostsFile = host.sshSettings.userKnownHostsFile.trimmed
        host.sshSettings.bindAddress = host.sshSettings.bindAddress.trimmed
        host.sshSettings.x11Display = host.sshSettings.x11Display.trimmed
        host.sshSettings.kexAlgorithms = host.sshSettings.kexAlgorithms.trimmed
        host.sshSettings.ciphers = host.sshSettings.ciphers.trimmed
        host.sshSettings.macs = host.sshSettings.macs.trimmed
        host.sshSettings.hostKeyAlgorithms = host.sshSettings.hostKeyAlgorithms.trimmed
        host.sshSettings.rekeyLimit = host.sshSettings.rekeyLimit.trimmed
        host.sshSettings.remoteCommand = host.sshSettings.remoteCommand.trimmed
        host.sshSettings.proxyHost = host.sshSettings.proxyHost.trimmed
        host.sshSettings.proxyUsername = host.sshSettings.proxyUsername.trimmed
        host.sshSettings.proxyPassword = host.sshSettings.proxyPassword.trimmed
        host.sshSettings.proxyCommand = host.sshSettings.proxyCommand.trimmed
        host.sshSettings.logFilePath = host.sshSettings.logFilePath.trimmed

        host.sshSettings.terminalColumns = max(40, min(400, host.sshSettings.terminalColumns))
        host.sshSettings.terminalRows = max(10, min(200, host.sshSettings.terminalRows))
        host.sshSettings.scrollbackLines = max(0, min(1_000_000, host.sshSettings.scrollbackLines))
        host.sshSettings.fontSize = max(9, min(30, host.sshSettings.fontSize))
        host.sshSettings.connectTimeoutSeconds = max(0, min(600, host.sshSettings.connectTimeoutSeconds))
        host.sshSettings.keepAliveIntervalSeconds = max(0, min(3600, host.sshSettings.keepAliveIntervalSeconds))
        host.sshSettings.keepAliveCountMax = max(1, min(20, host.sshSettings.keepAliveCountMax))
        host.sshSettings.proxyPort = max(0, min(65535, host.sshSettings.proxyPort))
