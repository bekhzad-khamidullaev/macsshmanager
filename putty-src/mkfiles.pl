...
# need to generate those _now_, before attempting dependency analysis.
{
    my $cwd = getcwd();
    eval {
        chdir "charset" or die "unable to chdir into charset\n";
        do "./sbcsgen.pl" or die($@ || $!);
        1;
    };
    chdir $cwd;
}
eval { do "./licence.pl"; 1; };
select STDOUT;
...
foreach $i (@prognames) {
  ...
  foreach $j (@list) {
    ...
    if ($j =~ /^(.*)\.res$/) {
      ...
      $file = "$j.c";
      if (!&findfile($file)) {
        print STDERR "mkfiles: using .m for $j\n" if $ENV{'MKFILES_DEBUG'};
        $file = "$j.m";
      }
      $depends{$j} = [$file];
      push @scanlist, $file;
...
while (scalar @scanlist > 0) {
  ...
  if (!defined $dirfile && $file =~ /^(.*)\.m$/) {
      $dirfile = &findfile("$1.c");
  }
  die "unable to locate source file $file\n" unless defined $dirfile;
